...
export function defaultPreviewSize(minimum, current) {
    return [
        Math.max(current[0], minimum[0], DEFAULT_PREVIEW_WIDTH),
        Math.max(current[1], minimum[1] + DEFAULT_PREVIEW_EXTRA_HEIGHT),
    ];
}
export function shouldExpandDefaultPreview(initial, current, loaded, previewHandled) {
    return !loaded && !previewHandled && sameSize(initial, current);
}
function handlePreviewWidget(node, widget, state) {
    if (widget.name !== NATIVE_IMAGE_PREVIEW_WIDGET || state.previewHandled) {
        return;
    }
    const current = currentSize(node);
    const shouldExpand = shouldExpandDefaultPreview(state.initialSize, current, state.loaded, state.previewHandled);
...
export function installPreviewPolicy(node) {
...
export function markNodeLoaded(node) {
